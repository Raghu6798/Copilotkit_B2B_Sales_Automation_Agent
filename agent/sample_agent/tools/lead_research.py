...
from sample_agent.tools.base.search_tools import google_search
from sample_agent.tools.base.linkedin_tools import extract_linkedin_url, scrape_linkedin
...
def extract_company_name(email):
    logger.info("Extracting company name from email: {}", email)
    """
    ...
    try:
        ...
        logger.info("Extracted company name: {}", company_name)
        return company_name
    except IndexError:
        logger.error("Failed to extract company name from email: {}", email)
        return "Company not found"

def research_lead_on_linkedin(lead_name, lead_company):
    logger.info("Researching lead on LinkedIn: {} from {}", lead_name, lead_company)
    """
    ...
    company_name = extract_company_name(lead_company)
    logger.info("Extracted company name for LinkedIn search: {}", company_name)
    ...
    if not lead_linkedin_url:
        logger.warning("Lead LinkedIn URL not found for query: {}", query)
        return "Lead LinkedIn URL not found."
    logger.info("Found lead LinkedIn URL: {}", lead_linkedin_url)

    # Scrape lead LinkedIn profile
    linkedin_data = scrape_linkedin(lead_linkedin_url)
    if "data" not in linkedin_data:
        logger.error("LinkedIn profile not found for URL: {}", lead_linkedin_url)
        return "LinkedIn profile not found"
    logger.info("Scraped LinkedIn profile for URL: {}", lead_linkedin_url)
    ...
    logger.info("Extracted company details: Name={}, Website={}, LinkedIn={}", company_name, company_website, company_linkedin_url)
    ...
    logger.info("Generated lead profile summary.")
